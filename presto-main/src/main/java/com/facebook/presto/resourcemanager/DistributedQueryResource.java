...
public class DistributedQueryResource
{
            ...
            @QueryParam("limit") Integer limitFilter)
    {
        ...
        List<BasicQueryInfo> sortingQueryList = new ArrayList<>(queries);
        if (limit < queries.size()) {
            sortingQueryList.sort(QUERIES_ORDERING);
        }
        else {
            limit = queries.size();
        }
        ImmutableList.Builder<BasicQueryInfo> builder = new ImmutableList.Builder<>();
        sortingQueryList = builder.addAll(sortingQueryList.subList(0, limit)).build();
        return Response.ok(sortingQueryList).build();
    }
